...
- **CORE-R-260 Pure prepared-commit services:** mutating M04 service functions MUST return deterministic `PreparedCommitV1` values and MUST NOT perform persistence, network, process, repository, HIVE/GitHub or clock operations.
...
- **CORE-R-265 Concrete V1 service surface:** the public implementation MUST provide explicit admission, attempt creation, step declaration, transition, cancellation, continuation, reference attachment, replay and snapshot verification operations with typed V1 requests/results/errors.
