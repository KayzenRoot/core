...
- **CORE-R-209 Exact initial file map:** initial M03 implementation MUST remain inside the frozen `crates/core-work-order` file topology plus the explicitly listed root/fuzz/workflow/evidence integration files unless a governed Correction Delta admits more.
- **CORE-R-210 Final initial dependency set:** `core-work-order` direct dependencies MUST be limited to core-contracts, core-identity, core-workspace, serde, serde_json and thiserror for V0.0 unless governed evidence admits another dependency.
- **CORE-R-211 No direct M03 hash implementation:** `core-work-order` MUST NOT depend directly on sha2 or introduce another generic canonical hashing stack; semantic hashing remains through core-identity.
- **CORE-R-212 M03-owned resource contract:** `M03ResourceBudget` and `M03ResourceUsageV1` MUST be owned by core-work-order and passed explicitly at service boundaries; core-config is not an initial M03 dependency.
- **CORE-R-213 External resolution data boundary:** source/Git/HIVE/GitHub/filesystem resolution MUST cross M03 through typed resolved evidence DTOs rather than hidden resolver/process/network/filesystem calls inside the compiler.
- **CORE-R-214 No resolver I/O trait in compiler:** V0.0 WorkOrderService MUST NOT accept a SourceResolver/LineageStore/Git/HIVE/network I/O trait or callback that could smuggle ambient authority into compile/validate/diff/classify/admit/handoff operations.
- **CORE-R-215 Explicit source resolution evidence:** compile MUST consume `ResolvedSourceSetV1` whose source IDs, semantic fingerprints, provenance fingerprints, freshness and secret classification correspond to the declared canonical source manifest.
- **CORE-R-216 Ephemeral workspace admission evidence:** M03 admission MAY consume current M02 `WorkspaceHandleV1`/generation/fingerprint evidence, but no live handle may enter FrozenWorkOrder semantic identity.
- **CORE-R-217 Context Lock proof contract:** admission MUST validate a concrete `ContextLockAdmissionProofV1` against the frozen requirement, WorkOrderId/revision/fingerprint, source manifest/base binding and implementation-authorization requirement.
- **CORE-R-218 Governance proof contract:** execution-capable admission MUST require an externally minted `GovernanceAdmissionProofV1` with APPROVED verdict and exact frozen/policy/base binding when the governance requirement is blocking.
- **CORE-R-219 Closed v1 envelope kinds:** M03 envelope schema/version/kind MUST be validated before semantic processing; unknown kinds or versions MUST fail typed and MUST NOT downgrade to v1.
- **CORE-R-220 Strong semantic ID types:** Work Order, packet, criterion, evidence, source, lock and governance IDs MUST use distinct bounded newtypes; duplicate IDs in one semantic namespace MUST fail before freeze.
- **CORE-R-221 Semantic string stability:** v1 semantic IDs/text are case-sensitive UTF-8 byte sequences and MUST NOT undergo hidden Unicode normalization/case folding absent explicit field-specific future policy.
- **CORE-R-222 Deterministic packet normalization:** packet declarations MUST be persisted in deterministic topological order with stable packet-ID tie breaking after duplicate/dangling/cycle/scope validation.
- **CORE-R-223 Explicit semantic projection:** WorkOrderFingerprint MUST hash only the frozen semantic projection; stored fingerprint, receipts, wall-clock timing and mutable diagnostics MUST NOT recursively alter that identity.
- **CORE-R-224 Timestamp-free semantic DCR:** CompilationReceiptV1 semantic identity MUST omit wall-clock timestamps; host logs MAY record time outside the semantic receipt.
- **CORE-R-225 Exact AEG structural gate:** every blocking acceptance criterion MUST have required evidence edges or a typed governed N/A rule; orphan blocking criteria MUST fail compile/validation.
- **CORE-R-226 PCM canonical ordering:** PacketContextMesh shared sources and packet edge sets MUST be sorted, duplicate-free and losslessly reconstruct every packet's mandatory source set.
- **CORE-R-227 Frozen public error contract:** WorkOrderError MUST expose closed v1 category, reason, retryability and bounded safe diagnostics; unknown internal state MUST fail as internal invariant, never READY.
- **CORE-R-228 Frozen M03 fuzz set:** implementation MUST include the seven Round 4 M03 fuzz targets unless a governed Correction Delta changes the set.
- **CORE-R-229 Frozen M03 property laws:** implementation MUST prove the twenty Round 4 property laws, including compile replay/permutation, DAG/scope/AEG/PCM, lineage/admission/correction/resource/cache/secret/no-hidden-I/O laws.
- **CORE-R-230 Frozen benchmark families:** M03 implementation MUST benchmark validation, canonicalization, compile, frozen validation, packet DAG, AEG, PCM, diff, correction classification, admission, serialization and optional memo behavior over the frozen scaling axes.
- **CORE-R-231 M03 Resource Calibration Gate:** exact production M03ResourceBudget defaults and absolute benchmark limits MUST be produced by a mandatory deterministic implementation-time calibration stage before M03 production acceptance.
- **CORE-R-232 Bounded M03 Calibration Delta:** the M03 Work Order MAY authorize a post-measurement delta limited to numeric budget defaults, benchmark-derived absolute thresholds, fixture metadata/evidence refs and explanatory docs; contracts/dependencies/file map/security/DoD cannot change through calibration.
- **CORE-R-233 Compatible PRB policy reuse:** M03 benchmark regression acceptance MUST reuse the repository's governed compatible PRB policy rather than introduce a second arbitrary regression percentage in module code/docs.
- **CORE-R-234 Honest unsupported-scale reporting:** benchmark/calibration scenarios that cannot run within bounded host resources MUST be recorded as bounded/skipped, not extrapolated into PASS.
- **CORE-R-235 No-hidden-I/O evidence:** implementation acceptance MUST include static/dependency/API evidence plus tests demonstrating that M03 core service operations do not invoke process/network/Git/HIVE/filesystem discovery.
- **CORE-R-236 No new third-party dependency by convenience:** regex/graph/Git/database/cache/watcher/HIVE/LLM/network/async crates are not admitted for initial M03; inability to meet a frozen obligation requires a dependency-admission Correction Delta.
- **CORE-R-237 Strict M04 handoff match:** AdmittedWorkOrderV1 materialization MUST require READY receipt plus exact WorkOrderId/revision/fingerprint match; mismatch/stale proof fails typed.
- **CORE-R-238 Initial lower-crate stability:** initial M03 implementation is not expected to modify core-workspace, core-runtime, core-cli, core-identity, core-contracts or core-config product code; any required lower-crate change MUST be separately justified and governed.
- **CORE-R-239 Round 4 residual limit:** after Round 4, only evidence-derived numeric M03 budgets/absolute thresholds and final DoD/Work Order/Context Lock/planning-freeze artifacts may remain unresolved before implementation authorization.
