...
## M04 Round 4 implementation-addressable requirements

- **CORE-R-256 One-crate initial implementation:** M04 V0.0 MUST be implemented as one focused `core-run-state` crate unless later governed evidence proves a split necessary.
...
- **CORE-R-264 Caller/deterministic identity creation:** semantic Run/Attempt/Step/Event identities MUST be caller-supplied or deterministically derived from versioned logical inputs; core MUST NOT use random or wall-clock generation.
- **CORE-R-265 Concrete V1 service surface:** the public implementation MUST provide explicit pure prepare functions for admission, attempt creation, step declaration, transition, cancellation, continuation and reference attachment; pure replay/snapshot verification; and a pure `finalize_commit` verifier that releases the operation-specific Round 3 receipt only after a matching `DurableCommitReceiptV1` proves atomic persistence.
- **CORE-R-266 Store/replay separation:** projection/replay and transition preparation MUST be independently testable in memory without a concrete storage backend.
- **CORE-R-267 Six bounded M04 fuzz targets:** implementation MUST provide the frozen transition-event, replay-journal, identity-canonical, BRC-ICF, reference and resource-limit fuzz surfaces in the existing fuzz package.
- **CORE-R-268 Exact property law inventory:** implementation MUST prove the Round 4 transition, terminal immutability, parent closure, CAS, idempotency, cancellation, replay/root integrity, BRC/ICF, snapshot, resource atomicity and reference-boundary laws.
...
## M04 Round 5 final-freeze requirements

- **CORE-R-275 Final execution packet is explicit and immutable-by-admission:** Round 5 MUST freeze one Work Order, one pending Context Lock, one Evidence Bundle skeleton, one executor handoff and one exact acceptance/evidence mapping for M04 V0.0. Later execution admission MAY bind canonical base/authorization state only and MUST NOT silently change frozen semantics.
- **CORE-R-276 Pending Context Lock is non-authoritative:** the Round 5 Context Lock MUST remain `PENDING_PROMOTION`, with `authorizedBase = null` and `productImplementationAuthorized = false`. A planning branch MUST NOT grant implementation authority.
- **CORE-R-277 Separate execution admission is mandatory:** M04 product code MUST NOT begin until the final-freeze candidate is independently reviewed/promoted and a later distinct admission delta activates the exact lock on canonical `origin/main`.
- **CORE-R-278 Construction packet order is frozen:** the future implementation MUST execute ordered Packs A-H: contracts/identity, lifecycle/projection, journal/replay/snapshot, CAS/idempotency/cancellation, BRC/ICF/references, resource/store/purity, security/property/fuzz/supply-chain, then calibration/evidence/CI/handoff. A dependent pack MUST NOT proceed while an upstream blocking obligation is unresolved.
- **CORE-R-279 Acceptance mapping is one-to-one and blocking:** AC-M04-001 through AC-M04-023 MUST map one-to-one to EV-M04-001 through EV-M04-023. No blocking node may be silently omitted, merged away or marked passed without exact-head evidence.
...
- **CORE-R-281 Executor has only two terminal handoff states:** the implementation executor MAY stop as `READY_FOR_REVIEW` only when all frozen packs, ACs and evidence gates are satisfied on one exact head, otherwise it MUST stop `BLOCKED` with the exact unresolved obligation. The executor MUST NOT self-approve.
- **CORE-R-282 Final reviewer owns approval:** only an independent exact-head governed review with zero unresolved HIGH/CRITICAL findings may produce APPROVED and permit implementation promotion.
